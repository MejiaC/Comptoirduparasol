@extends('layouts.common')
@section('content')
    <section id="content-produits">
        <h2 class="h2-4">Nos produits par catégorie</h2>
        <div class="span3">
            <div class="plan">
                <div class="title">
                    <h3><a href="/produits/abri">Abri</a><span></span></h3>
                </div>
                <div class="types-produits">
                    <ul class="produits">
                        <li><b>Tentes</b>
                        <li><b>Parasols de marché</b>
                        <li><b>Parasols de C.H.R</b>
                    </ul>
                </div>
            </div>
        </div>

        <div class="span3">
            <div class="plan">
                <div class="title">
                    <h3><a href="/produits/tables">Tables</a><span></span></h3>
                </div>
                <div class="types-produits">
                    <ul class="produits">
                        <li><b>Tables rondes</b></li>
                        <li><b>Tables rectangulaires</b></li>
                        <li><b>Tables guéridon</b></li>
                        <li><b>Tables mange debout</b></li>
                    </ul>
                </div>
            </div>
        </div>
        <div class="span3">
            <div class="plan">
                <div class="title">
                    <h3><span><a href="/produits/habillage">Habillage</a></span></h3>
                </div>
                <div class="types-produits">
                    <ul class="produits">
                        <li><b>Housse pour chaises</b></li>
                        <li><b>Housse pour lestages</b></li>
                        <li><b>Housse pour tables</b></li>
                    </ul>
                </div>
            </div>
        </div>
        <div class="span3">
            <div class="plan">
                <div class="title">
                    <h3><a href="/produits/chaises">Chaises</a><span></span></h3>
                </div>
                <div class="types-produits">
                    <ul class="produits">
                        <li><b>Chaises Napoléon</b>
                        <li><b>Chaises à couleurs</b>
                        <li><b>Chaises coques</b>
                        <li><b>Tabouret</b>
                        <li><b>Fauteuil</b>
                    </ul>
                </div>
            </div>
        </div>
        <div class="span3">
            <div class="plan">
                <div class="title">
                    <h3><a href="/produits/lestages">Lestages</a><span></span></h3>
                </div>
                <div class="types-produits">
                    <ul class="produits">
                        <li><b>Lestages en fonte</b></li>
                        <li><b>Lestages en béton</b></li>
                    </ul>
                </div>
            </div>
        </div>
        <div class="span3">
            <div class="plan">
                <div class="title">
                    <h3><a href="/produits/protection">Protection</a><span></span></h3>
                </div>
                <div class="types-produits">
                    <ul class="produits">
                        <li><b>Poteaux de guidage</b></li>
                        <li><b>Porte messages</b></li>
                        <li><b>Barrière de sécurité</b></li>
                    </ul>
                </div>
            </div>
        </div>

        <div class="span3">
            <div class="plan">
                <div class="title">
                    <h3><span><a href="/produits/scened">Scène</a></span></h3>
                </div>
                <div class="types-produits">
                    <ul class="produits">
                        <li><b>Podiums</b></li>
                        <li><b>Planché</b></li>
                    </ul>
                </div>
            </div>
        </div>
        <div class="span3">
            <div class="plan">
                <div class="title">
                    <h3><span><a href="/produits/chauffage">Appareil chaufant</a></span></h3>
                </div>
                <div class="types-produits">
                    <ul class="produits">
                        <li><b>Chauffage à radion de gaz</b></li>
                        <li><b>Chauffage à radion électrique</b></li>
                        <li><b>Pied de chauffage</b></li>
                    </ul>
                </div>
            </div>
        </div>
    </section>
@stop
